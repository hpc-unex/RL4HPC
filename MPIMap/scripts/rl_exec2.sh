...
mv -fi $OUTPUT_FOLDER/output.txt $OUTPUT_FOLDER/output_`date +"%Y-%m-%d_%H-%M"`.txt
...
OMPIALG=" --mca coll_tuned_priority 100 --mca coll_tuned_use_dynamic_rules 1 --mca coll_tuned_bcast_algorithm $BCAST_BINOMIAL_ALG "
...
# FALTA:  " --hostfile " + params["hosts_file"] + " --rankfile " + params["rank_file"]
/opt/openmpi_gcc/2.0.2/bin/mpirun -n $numProcs --mca btl $COMMS $OMPIALG --bind-to core -report-bindings --display-map -nooversubscribe  python3 $SCRIPT_FOLDER/rl.py "$M" "$numProcs" "$net" "$m" "$BIN_FOLDER/IMB-MPI1" "$IMB_OPTIONS" "$DATA_FOLDER/graph_file.txt" "$DATA_FOLDER" "$OUTPUT_FOLDER/output.txt"
